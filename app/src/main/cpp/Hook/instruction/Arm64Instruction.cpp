...
void *A64HookFunctionV(void *const symbol, void *const replace,
                       void *const rwx, const uintptr_t rwx_size, HookInfo *hookInfo) {
    static constexpr uint_fast64_t mask = 0x03ffffffu; // 0b00000011111111111111111111111111
    ...
    if (llabs(pc_offset) >= (mask >> 1)) {
        ...
        hookInfo->setJumpStubLen(count * sizeof(int32_t));
        if (trampoline) {
            ...
            hookInfo->setCallOriginalIns((uint8_t *) trampoline);
        } //if
        uint32_t *temp = new uint32_t[5];
        memset(temp, 0, 5 * sizeof(uint32_t));

        if (count == 5) {
            temp[0] = A64_NOP;
            ++temp;
        } //if
        temp[0] = 0x58000051u; // LDR X17, #0x8
        temp[1] = 0xd61f0220u; // BR X17
        *reinterpret_cast<int64_t *>(temp + 2) = __intval(replace);
        //__flush_cache(symbol, 5 * sizeof(uint32_t));
        hookInfo->setJumpStubBack((uint8_t *) temp);
        A64_LOGI("inline hook %p->%p successfully! %zu bytes overwritten",
                 symbol, replace, 5 * sizeof(uint32_t));

    } else {
        hookInfo->setJumpStubLen(1 * sizeof(int32_t));
        if (trampoline) {
            if (rwx_size < 1u * 10u) {
        ...
        uint32_t *temp = new uint32_t[1];
        memset(temp, 0, 1 * sizeof(uint32_t));

        __sync_cmpswap(temp, *temp,
...
void A64HookFunction(void *const symbol, void *const replace, HookInfo *hookInfo) {
    ...
    if (trampoline != nullptr) {
        ...
        LOGE("hook symbol:%p fail", symbol);
    }
}


bool Arm64Instruction::createStub(HookInfo *info) {

    void *replace = info->getHookAddr();
    void *symbol = info->getOriginalAddr();
    LOGI("createStub replace:%p symbol:%p", replace, symbol);
    A64HookFunction(symbol, replace, info);
    return true;
}


bool Arm64Instruction::createCallOriginalStub(HookInfo *info) {
    void *replace = info->getHookAddr();
    void *symbol = info->getOriginalAddr();
    return true;
}

bool Arm64Instruction::repairCallOriginIns(HookInfo *info,
                                           uint8_t *repair,
                                           uint32_t &repairLen) {

    /*   int32_t count = info->getJumpStubLen();
       void *symbol = info->getOriginalAddr();
